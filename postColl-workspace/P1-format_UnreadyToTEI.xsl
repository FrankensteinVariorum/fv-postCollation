...
<xsl:output method="xml" indent="yes"/>    
    <xsl:variable name="collFiles" as="document-node()+" select="collection('unready-collated-data/?select=*.xml')"/>
   ...
   <xsl:template match="/">    
       <xsl:for-each select="$collFiles//cx:apparatus"> 
           ...
           <xsl:result-document method="xml" indent="yes" href="unready-TEI/unr_{$chunk}.xml">
           <TEI xml:id="unr-{$chunk}">
            <teiHeader>
                <fileDesc>
                    <titleStmt>
...
<xsl:template match="app">
   ...
   <app  xml:id="{$chunk}_app{count(preceding::app) + 1}"><xsl:apply-templates>
    ...
    <xsl:template match="rdgGrp">
        <xsl:param name="chunk" tunnel="yes"/>
        <rdgGrp xml:id="{$chunk}_app{count(preceding::app) + 1}_rg{count(preceding-sibling::rdgGrp) + 1}" n="{@n}"><xsl:apply-templates/></rdgGrp>
    </xsl:template>
