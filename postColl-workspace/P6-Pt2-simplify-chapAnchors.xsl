...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="P6-Pt1" as="document-node()+" select="collection('P6-Pt1/?select=*.xml')"/> 
     ...
     <xsl:when test="not($witness = 'f1831') and not($witness = 'fMS')">   
      ...
      <xsl:choose>
        <xsl:when test="$chapterMarker/preceding::tei:milestone[@unit='volume'][1]">
          <xsl:value-of select="concat('_vol_', $chapterMarker/preceding::tei:milestone[@unit='volume'][1]/@n)"/>
  ...
  <xsl:template match="/">
    <xsl:for-each select="$P6-Pt1">
      ...
      <xsl:result-document method="xml" indent="yes" href="P6-Pt2/{$filename}">
        <TEI>
          <teiHeader>
            <fileDesc>
              <titleStmt>
  ...
  <xsl:template match="tei:seg[tei:milestone[@unit='chapter' and @type='start']]">
    ...
    <xsl:param name="witness"/>
    <xsl:variable name="chapterMarker" as="element()" select="tei:milestone[@unit='chapter' and @type='start']"/>
    ...
    <xsl:variable name="chap_id" as="xs:string" select="following::tei:head[1]/following-sibling::text()[1] ! lower-case(.) ! replace(., '[.,:;]', '') ! tokenize(., ' ') => string-join('_') 
      ...
      <anchor type="semantic" subtype="{tei:milestone/@type}" xml:id="{$witness}{$vol_info}_{$chap_id}"/>
  ...
  <xsl:template match="sga-add"><!-- ebb: Let's restore <sga-add> to just the TEI <add> element now that the collation is finished. -->
    <add>
      <xsl:copy-of select="@*"/>
      <xsl:apply-templates/>
    </add>
