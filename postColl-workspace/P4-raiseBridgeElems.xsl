...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
	...
	* and recur in default (unnamed) mode. *-->
    ...
    <xsl:function name="th:raise">
        ...
        <xsl:message>raise() called with <xsl:value-of select="count($input//*)"/>-element document
    ...
    <xsl:template match="/">
        <xsl:for-each select="$novel-coll//TEI">
            <xsl:variable name="filename">
                ...
                <xsl:value-of select="tokenize(base-uri(), '/')[last()] ! substring-after(., 'P3-')"
                />
            </xsl:variable>
            <xsl:variable name="chunk" as="xs:string"
                select="substring-after(substring-before(tokenize(base-uri(), '/')[last()], '.'), '_')"/>
            <xsl:result-document method="xml" indent="yes" href="P4-output/{$filename}">
                <TEI>
    ...
    <xsl:template match="titleStmt/title">
        <title>
            <xsl:text>Bridge Phase 4:</xsl:text>
    ...
    <xsl:template match="
        ...
        <xsl:element name="{name()}">
            ...
            <xsl:variable name="end-marker" as="element()"
                select="following::*[@th:eID = current()/@th:sID]"/>
    ...
    <xsl:template
        match="*[not(name() = 'mod') and not(name() = 'zone') and not(name() = 'sga-add')][@th:eID eq preceding::*[@th:sID][1]/@th:sID]"
    />
</xsl:stylesheet>
