...
<xsl:stylesheet 
    ...
    <xsl:template match="/">
        ...
        <!--    <xsl:variable name="chunk" as="xs:string" select="tokenize($filename, '_')[last()]"/>
            <xsl:result-document method="xml" indent="yes" href="P3-output/{$filename}">-->
                    ...
                    <xsl:apply-templates select="parse-xml($tester)" mode="raise"/>
            <!--</xsl:result-document>-->
        <!--</xsl:for-each>-->
    </xsl:template>
  ...
  <!--  <xsl:template match="body">
        ...
        <div type="collation" xml:id="{ab/@xml:id}">
           <!-\- <xsl:variable name="abDoc" as="document-node()" select="document(ab)"/>-\->
            <xsl:apply-templates select="parse-xml()" mode="raise"/>
        </div>
    </xsl:template>-->
