...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="P5-Pt4-coll" as="document-node()+"
    select="collection('P5-Pt4-output/?select=*.xml')"/>
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="$P5-Pt4-coll/*">
      ...
      <xsl:result-document method="xml" indent="yes" href="P5-Pt5-output/{$filename}">
