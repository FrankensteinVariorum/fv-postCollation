    ...
    * When edit distances are calculated and stored, run spine_addLevWeights.xsl to add Levenshtein values and generate the finished standoff_Spine directory files.
    ...
    <xsl:template name="lookback">
        ...
        <xsl:variable name="pointer">
            ...
            <xsl:value-of
                select="pitt:getLbPointer(normalize-space(tokenize($rdg/preceding::tei:rdg[ends-with(@wit, 'fMS')][contains(normalize-space(.), 'lb n=&quot;')][1], '&lt;lb\s+n')[last()]))"
            />
        </xsl:variable>
                                        ...
                                        <xsl:choose>
                                            ...
                                            <xsl:when test="starts-with(normalize-space(.), '=')">
                                                ...
                                                <xsl:if
                                                  ...
                                                  <xsl:value-of
                                                  select="pitt:getLbPointer(normalize-space(current()))"
                                                  />
                                                  </xsl:variable>
                                                  <xsl:if test="not($pointer = '')">
                                                  <xsl:variable name="text" select="
                                                  ...
                                                  select="concat($sga_loc, 'fMS_', $wholeChunkID, '.xml', '#')"/>
                                                  <xsl:value-of
                                                  select="concat(string-join(pitt:getLbPointer(normalize-space(current()))), ',0,', string-length($text) + 1, ')')"
                                                  />
                                                  </xsl:variable>
                    ...
                    <xsl:otherwise>
                        <rdg wit="{@wit}" xmlns="http://www.tei-c.org/ns/1.0">
                            ...
                            <xsl:for-each select="$currEd-Seg">
                                <ptr
                                    target="https://raw.githubusercontent.com/PghFrankenstein/fv-data/master/variorum-chunks/{$currWit}_{$currEd-Chunk}.xml#{current()/@xml:id}"
                                />
                            </xsl:for-each>
                        </rdg>
                    </xsl:otherwise>
