...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:variable name="P3-Coll" as="document-node()+"
    ...
    <xsl:template match="/">
        <xsl:for-each-group select="$P3-Coll"
            group-by="tokenize(base-uri(), '-f')[last()] ! tokenize(., '[a-z]?\.xml')[1]">
            <xsl:variable name="filename" select="concat('P3-f', current-grouping-key(), '.xml')"/>
            <xsl:variable name="witness"
                select="tokenize($filename, '-')[last()] ! substring-before(., '_')"/>
            ...
            <xsl:result-document method="xml" indent="yes" href="P3.5-output/{$filename}">
                <TEI>
                    <teiHeader>
                        <fileDesc>
                            <titleStmt>
                                <title>Bridge Phase 3: Witness <xsl:value-of select="$witness"/>,
                    ...
                    <text>
                        <body>
                            <div type="collation"
                                xml:id="{tokenize(descendant::div/@xml:id, '[a-z]')[1]}">
                                <xsl:for-each select="current-group()//TEI">
                                    <xsl:sort select="base-uri()"/>
                                    <xsl:apply-templates
                                        select="descendant::div[@type = 'collation']"/>
                                </xsl:for-each>
                            </div>
                        </body>
                    </text>
