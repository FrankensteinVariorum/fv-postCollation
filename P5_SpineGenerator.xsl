...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"  xmlns:pitt="https://github.com/ebeshero/Pittsburgh_Frankenstein"
    ...
    <xsl:template name="lookback">
        ...
        <xsl:variable name="str" select="tokenize(normalize-space(string-join($rdg/preceding::rdg[ends-with(@wit, 'fMS')])), '&lt;lb\s+n')[last()]"/>
        <xsl:variable name="pointer">
            <xsl:value-of select="pitt:getLbPointer(normalize-space(tokenize($rdg/preceding::rdg[ends-with(@wit, 'fMS')][contains(normalize-space(.), 'lb n=&quot;')][1], '&lt;lb\s+n')[last()]))"/>
        </xsl:variable>
