...
* This XSLT finds `<app>` elements with only two variations that amount to only a single 4-character token, which can only ever be `n=['']`. 
It ensures that these are output with a Levenshtein distance of 0. WAIT, this may NOT be what we want now!
